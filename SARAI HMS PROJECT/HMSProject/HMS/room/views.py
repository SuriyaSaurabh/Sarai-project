...
#room ke 6  api yha se shuru mere

@ login_required(login_url='login')
def rooms(request):
    role = str(request.user.groups.all()[0])   #retun  QuerySet Object first index
    path = role + "/"             #role = admin/ ,guest/
    rooms = Room.objects.all()      #objects ke sare value nekale le
    firstDayStr = None
    lastDateStr = None
    ...
    return render(request, path + "rooms.html", context)  #display krna
...
def add_room(request):
    ...
    if request.method == "POST":
        guest = None
        if role == 'guest':    #string check kr rhe h yha
...
def room_profile(request, id):
    role = str(request.user.groups.all()[0])  #retun  QuerySet Object ke  first index
    path = role + "/"
...
def room_services(request):
    ...
    room_services = RoomServices.objects.all()    #context means ek dictionary bna rhe h jo retun me return kr rhe h
    context = {
        "role": role,
...
def current_room_services(request):
    ...
    curBooking = Booking.objects.filter(guest=curGuest).last()   #last()  vale filed lake dega ye

    if curBooking is not None:
        curRoom = Room.objects.get(number=curBooking.roomNumber.number)    #room numbernumber se uske value jo ke room_id db me nekal rha h
    else:
        context = {
            "role": role,
            "error": "You Don't Have Booking Right Now"
        }
        return render(request, path + "current-room-services.html", context)     #context means ek dictionary bna rhe h jo retun me return kr rhe h
    ...
    availableEmployee = list()                                   # i create here blank list
    maxTaskNum = 10
    ...
    if request.method == "POST":
        if "foodReq" in request.POST:
            ...
            lastTask = Task.objects.filter(employee=chosenEmp).last()    #last()  vale filed lake dega ye
            if(lastTask != None):
            ...
            newTask.save()
            return redirect("current-room-services")    #us pe redirect pahle vale page aajata h
    ...
    return render(request, path + "current-room-services.html", context)     #context mtlab dictionary return ke


#mere api 6  jo yha khatm
